...
async function seedProductionDatabase() {
  try {
    ...
    // Common MongoDB Atlas connection strings - try these first
    const possibleUris = [
      process.env.MONGODB_URI,
      'mongodb+srv://username:password@cluster0.xxxxx.mongodb.net/mini-ecommerce?retryWrites=true&w=majority',
      'mongodb+srv://username:password@cluster0.xxxxx.mongodb.net/ecommerce_social?retryWrites=true&w=majority'
    ];
    ...
    for (const uri of possibleUris) {
      if (uri && uri !== 'mongodb+srv://username:password@cluster0.xxxxx.mongodb.net/mini-ecommerce?retryWrites=true&w=majority') {
        try {
    ...
    if (!connected) {
      console.log('❌ Could not connect to MongoDB Atlas automatically.');
      console.log('📝 Please provide your MongoDB Atlas connection string:');
      console.log('   1. Go to MongoDB Atlas → Clusters → Connect → Connect your application');
      console.log('   2. Copy the connection string');
      console.log('   3. Replace the connection string in this file');
      console.log('   4. Run: node fix-production-data.js');
      process.exit(1);
    }
    ...
    console.log('🌐 Your backend should now return data!');
    
    // Test the API
    console.log('\n🧪 Testing the API...');
    const testResponse = await fetch('https://mini-ecommerce-backend-ppue.onrender.com/api/products');
    const testData = await testResponse.json();
    console.log(`📊 API now returns ${testData.products.length} products`);
    ...
    console.log('   3. Check if your database user has read/write permissions');
    process.exit(1);
  }
}
